...
const PANEL_TAG = "homekit-preview-panel-v0103";
const BUILD_LABEL = "0.10.3 · confirm-dialog";
const EMPTY_FILTER = {
...
class HomeKitPreviewPanel extends HTMLElement {
  ...
  renderConfirmDialog() {
    ...
    return `<div class="confirmScrim" role="presentation">
      <section id="confirmDialog" class="confirmDialog ${this.escape(dialog.tone || "")}" role="dialog" aria-modal="true" aria-labelledby="confirmTitle" tabindex="-1">
        <div class="confirmTitle" id="confirmTitle">${this.escape(dialog.title)}</div>
        <div class="confirmBody">${this.escape(dialog.body)}</div>
  ...
  renderStyles() {
    return `<style>
      ...
      * { box-sizing: border-box; }
      ...
      .confirmDialog { width: min(520px, 100%); border: var(--hp-border); border-top: 4px solid var(--primary-color); border-radius: var(--hp-radius); background: var(--hp-surface); box-shadow: 0 18px 52px rgba(0, 0, 0, .32); padding: 16px; }
  ...
  bind(entry) {
    ...
    this.shadowRoot.getElementById("confirmDialog")?.addEventListener("keydown", (ev) => { if (ev.key === "Escape") this.resolveConfirm(false); });
