...
const PANEL_TAG = "homekit-preview-panel-v098";
const BUILD_LABEL = "0.9.8 · sticky-draft";
const EMPTY_FILTER = {
...
class HomeKitPreviewPanel extends HTMLElement {
  ...
  renderStyles() {
    return `<style>
      ...
      * { box-sizing: border-box; }
      ...
      .draftBar { position: sticky; bottom: 12px; z-index: 3; display: grid; grid-template-columns: minmax(220px, 1fr) auto; gap: 12px; align-items: center; padding: 14px; border: var(--hp-border); border-top: 3px solid var(--primary-color); border-radius: var(--hp-radius); background: color-mix(in srgb, var(--hp-surface) 94%, var(--primary-background-color)); box-shadow: 0 10px 26px rgba(0, 0, 0, .18); backdrop-filter: blur(8px); }
      ...
      .deviceCard { width: 100%; text-align: left; background: var(--hp-surface); color: var(--primary-text-color); border: var(--hp-border); border-radius: var(--hp-radius); padding: 12px; cursor: pointer; border-left: 4px solid transparent; overflow: hidden; white-space: normal; overflow-wrap: anywhere; }
      ...
      .deviceCardTop { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: start; gap: 8px; min-width: 0; }
      ...
      .deviceCue { flex: 0 0 auto; justify-self: end; }
      ...
      .optionCard { display: flex; align-items: flex-start; gap: 9px; padding: 11px; border: var(--hp-border); border-radius: var(--hp-radius); background: var(--hp-surface-soft); line-height: 1.35; }
      ...
      .sourceTile, .homeTile, .updateTile { border: var(--hp-border); border-radius: var(--hp-radius); background: var(--hp-surface-soft); padding: 14px; min-height: 138px; display: flex; flex-direction: column; justify-content: space-between; }
  ...
  renderSummary(data, entry, stats) {
    ...
    return `<div class="actionSummary">
      <div class="actionTile ${attentionTone}"><div class="actionIcon">${attentionIcon}</div><div><div class="actionTitle">${this.escape(attentionTitle)}</div><div class="actionNote">${this.escape(attentionNote)}</div></div></div>
      <div class="actionTile ${live ? "good" : "warn"}"><div class="actionIcon">${live}</div><div><div class="actionTitle">Live in Apple Home</div><div class="actionNote">${this.escape(entry?.title || "Selected bridge")}</div></div></div>
      <div class="actionTile ${unsupported ? "warn" : "good"}"><div class="actionIcon">${unsupported}</div><div><div class="actionTitle">Unsupported candidates</div><div class="actionNote">Filtered by HomeKit support rules.</div></div></div>
      <div class="actionTile ${stats.changed ? "warn" : "good"}"><div class="actionIcon">${stats.changed}</div><div><div class="actionTitle">Draft delta</div><div class="actionNote">${this.escape(draftNote)}</div></div></div>
    </div>
  ...
  renderProxyBuilder(entry) {
    ...
    return `<div class="panel">
      ...
      <div class="homeTileMeta"><div><b>Semantic note</b></div><div>${this.escape(profile.semantic_warning || "This helper changes HomeKit semantics while preserving the unit and value.")}</div><div class="inlineMeta"><span class="chip">Source ${this.escape(source.device_class || "sensor")} ${this.escape(source.unit_of_measurement || "")}</span><span class="chip">Helper ${this.escape(profile.device_class)} ${this.escape(profile.unit || "")}</span><span class="chip">Same unit ${this.escape(profile.unit || source.unit_of_measurement || "")}</span></div></div>
