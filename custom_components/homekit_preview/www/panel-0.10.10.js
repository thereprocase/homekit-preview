...
const PANEL_TAG = "homekit-preview-panel-v0110";
const BUILD_LABEL = "0.10.10 · mobile-toolbar";
const EMPTY_FILTER = {
...
class HomeKitPreviewPanel extends HTMLElement {
  ...
  devicePriority(device) {
    const proxyReady = device.entities.some((entity) => entity.selectable === false && this.proxyProfilesFor(entity).length);
    const dropped = device.entities.some((entity) => entity.selectable === false && !entity.currently_exposed);
    const live = device.entities.some((entity) => entity.currently_exposed);
    if (proxyReady) return 0;
    if (dropped) return 1;
    if (live) return 2;
    return 3;
  }

  devicePriorityCounts(devices) {
    const counts = { proxyReady: 0, dropped: 0, liveOnly: 0, other: 0 };
    for (const device of devices || []) {
      const proxyReady = device.entities.some((entity) => entity.selectable === false && this.proxyProfilesFor(entity).length);
      const dropped = device.entities.some((entity) => entity.selectable === false && !entity.currently_exposed);
      const live = device.entities.some((entity) => entity.currently_exposed);
      if (proxyReady) counts.proxyReady += 1;
      if (dropped) counts.dropped += 1;
      if (live && !proxyReady && !dropped) counts.liveOnly += 1;
      if (!live && !proxyReady && !dropped) counts.other += 1;
    }
    return counts;
  }
  ...
  devices(entry) {
    ...
    const devices = Array.from(groups.values()).sort((a, b) => this.devicePriority(a) - this.devicePriority(b) || a.room.localeCompare(b.room) || a.name.localeCompare(b.name));
  ...
  renderStyles() {
    return `<style>
      ...
      * { box-sizing: border-box; }
      ...
      .actionSummary { display: grid; grid-template-columns: minmax(260px, .95fr) minmax(320px, 1.35fr); gap: 10px; margin: 0 0 10px; align-items: stretch; }
      ...
      .primaryAction { min-height: 100%; }
      .quickActions { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; min-width: 0; }
      .quickAction { appearance: none; width: 100%; min-height: 78px; display: grid; grid-template-columns: auto minmax(0, 1fr); gap: 8px; align-items: start; text-align: left; border: var(--hp-border); border-radius: var(--hp-radius); background: var(--hp-surface); color: var(--primary-text-color); box-shadow: var(--hp-shadow); padding: 10px; white-space: normal; overflow-wrap: anywhere; }
      .quickAction:hover { border-color: color-mix(in srgb, var(--primary-color) 42%, var(--divider-color)); background: color-mix(in srgb, var(--primary-color) 5%, var(--hp-surface)); }
      .quickActionIcon { width: 26px; height: 26px; border-radius: 999px; display: grid; place-items: center; background: var(--hp-surface-soft); color: var(--primary-text-color); font-weight: 900; font-size: 12px; line-height: 1; }
      .quickAction > span:last-child { min-width: 0; display: grid; gap: 3px; }
      .quickActionTitle { display: block; font-weight: 850; line-height: 1.2; overflow-wrap: anywhere; }
      .quickActionNote { display: block; color: var(--secondary-text-color); font-size: 12px; line-height: 1.3; overflow-wrap: anywhere; }
      ...
      .diagnosticStrip { display: flex; gap: 7px; flex-wrap: wrap; align-items: center; margin: 0; padding: 9px 10px; border: var(--hp-border); border-radius: var(--hp-radius); background: var(--hp-surface-soft); }
      .diagnosticLabel { color: var(--secondary-text-color); font-size: 12px; font-weight: 800; }
      .summaryDetails { margin: 0 0 var(--hp-gap); border: var(--hp-border); border-radius: var(--hp-radius); background: var(--hp-surface); box-shadow: var(--hp-shadow); overflow: hidden; }
      .summaryDetails summary { display: flex; align-items: center; justify-content: space-between; gap: 10px; padding: 10px 12px; cursor: pointer; list-style: none; font-weight: 850; min-width: 0; }
      .summaryDetails summary::-webkit-details-marker { display: none; }
      .summaryDetails summary::after { content: "Open"; flex: 0 0 auto; min-height: 24px; padding: 4px 8px; border: var(--hp-border); border-radius: 999px; color: var(--secondary-text-color); background: var(--hp-surface-soft); font-size: 12px; line-height: 1.2; font-weight: 800; }
      .summaryDetails[open] summary::after { content: "Close"; }
      .summaryDetailsTitle { min-width: 0; overflow-wrap: anywhere; }
      .summaryDetailsMeta { display: flex; gap: 6px; flex-wrap: wrap; justify-content: flex-end; min-width: 0; margin-left: auto; }
      .summaryDetailsBody { display: grid; gap: 10px; padding: 0 12px 12px; }
      .summaryDetailsBody .overviewGrid { margin: 0; }
      ...
      .sectionMeta .chip { max-width: min(260px, 100%); overflow: visible; text-overflow: clip; white-space: normal; overflow-wrap: anywhere; }
      ...
      .deviceCue { flex: 0 0 auto; justify-self: end; justify-content: center; max-width: 112px; text-align: center; }
      ...
      .controlBar { display: grid; grid-template-columns: minmax(122px, 190px) minmax(220px, 1fr) max-content; gap: 7px; padding: 7px; margin-bottom: 12px; align-items: center; }
      ...
      .sortSummary { display: flex; align-items: center; gap: 7px; flex-wrap: wrap; margin: -5px 0 12px; padding: 8px 10px; border: var(--hp-border); border-left: 4px solid var(--primary-color); border-radius: var(--hp-radius); background: var(--hp-surface); color: var(--secondary-text-color); font-size: 12px; line-height: 1.3; }
      .sortSummary strong { color: var(--primary-text-color); }
      .resetShort { display: none; }
      ...
      @media (max-width: 980px) {
        :host { padding: 14px; }
        .top, .layout, .proxySummary, .proxyFlow, .draftBar, .debugGrid, .actionSummary { grid-template-columns: 1fr; }
      ...
      @media (max-width: 620px) {
        ...
        .summaryDetails summary { align-items: flex-start; flex-direction: column; }
        .summaryDetailsMeta { margin-left: 0; justify-content: flex-start; }
        ...
        .controlBar { grid-template-columns: minmax(0, 1fr) minmax(84px, max-content); gap: 7px; padding: 6px; }
        .controlField { align-items: center; gap: 5px; }
        .searchField { grid-column: 1 / -1; }
        .fieldLabel { width: 44px; padding-top: 0; }
        .filterReset { align-self: stretch; width: auto; min-width: 84px; grid-column: 2; grid-row: 1; padding-inline: 7px; }
        .filterSummary { margin-top: -4px; }
        .filterSummary .filterReset { margin-left: 0; }
        .resetFull { display: none; }
        .resetShort { display: inline; }
        .deviceList { max-height: none; }
        .deviceCardTop { grid-template-columns: minmax(0, 1fr); }
        .deviceCue { justify-self: start; max-width: 100%; }
        .draftBar { bottom: 8px; padding: 10px; }
        .quickActions { grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 6px; }
        .quickAction { min-height: 52px; grid-template-columns: 1fr; justify-items: center; align-items: center; gap: 4px; padding: 8px 5px; text-align: center; }
        .quickActionIcon { width: 24px; height: 24px; }
        .quickAction > span:last-child { justify-items: center; gap: 0; }
        .quickActionTitle { font-size: 12px; line-height: 1.15; }
        .quickActionNote { display: none; }
  ...
  renderSummary(data, entry, stats) {
    ...
    const liveNote = `${live} live on ${entry?.title || "selected bridge"}`;
    const unsupportedNote = unsupported ? `${unsupported} filtered by HomeKit support rules` : "No unsupported candidates detected";
    const draftActionNote = stats.changed ? draftNote : "Draft matches live runtime";
    return `<div class="actionSummary">
      <div class="actionTile primaryAction ${attentionTone}"><div class="actionIcon">${attentionIcon}</div><div><div class="actionTitle">${this.escape(attentionTitle)}</div><div class="actionNote">${this.escape(attentionNote)}</div>${attentionAction}</div></div>
      <div class="quickActions" aria-label="HomeKit Preview quick actions">
        <button class="quickAction" data-jump-tab="preview" data-jump-target="liveEntities"><span class="quickActionIcon">${live}</span><span><span class="quickActionTitle">Live list</span><span class="quickActionNote">${this.escape(liveNote)}</span></span></button>
        <button class="quickAction" data-jump-tab="device" data-jump-target="devicePicker"><span class="quickActionIcon">${unsupported}</span><span><span class="quickActionTitle">Device picker</span><span class="quickActionNote">${this.escape(unsupportedNote)}</span></span></button>
        <button class="quickAction" data-jump-tab="device" data-jump-target="draftBar"><span class="quickActionIcon">${stats.changed}</span><span><span class="quickActionTitle">Exact draft</span><span class="quickActionNote">${this.escape(draftActionNote)}</span></span></button>
      </div>
    </div>
    <details class="summaryDetails">
      <summary><span class="summaryDetailsTitle">Metrics and diagnostics</span><span class="summaryDetailsMeta"><span class="chip">${live} live</span><span class="chip ${drops ? "hkDrop" : ""}">${drops} HK drops</span><span class="chip ${proxyReady ? "warnChip" : ""}">${proxyReady} proxy-ready</span><span class="chip">${stats.draft} draft</span></span></summary>
      <div class="summaryDetailsBody">
  ...
  renderControls(rooms, tab) {
    ...
    return `<div class="controlBar">
      ...
      <button class="secondary filterReset" data-clear-filters="${this.escape(tab)}" ${hasFilters ? "" : "disabled"}><span class="resetFull">Clear filters</span><span class="resetShort">Clear</span></button>
    </div>`;
  }
  ...
  renderDeviceSortSummary(devices) {
    if (!devices.length) return "";
    const counts = this.devicePriorityCounts(devices);
    return `<div class="sortSummary"><span><strong>Attention first</strong>: proxy-ready and HK-drop devices are pinned above live-only devices.</span><span class="chip warnChip">${counts.proxyReady} proxy-ready</span><span class="chip hkDrop">${counts.dropped} HK drops</span><span class="chip">${counts.liveOnly} live-only</span></div>`;
  }
  ...
  renderDevicePicker(entry, rooms, devices, selectedDevice, stats) {
    ...
    const deviceCards = devices.map((device) => {
      ...
      const cueClass = proxyReady ? "attention" : dropped ? "dropAttention" : "";
      const cue = proxyReady ? `<span class="statusBadge warn deviceCue">${proxyReady} proxy-ready</span>` : dropped ? `<span class="statusBadge hkDrop deviceCue">${dropped} dropped</span>` : live ? `<span class="statusBadge good deviceCue">live</span>` : `<span class="statusBadge deviceCue">not live</span>`;
    ...
    return `<div id="devicePicker" class="stack" tabindex="-1">
      ...
      ${this.renderDeviceSortSummary(devices)}
